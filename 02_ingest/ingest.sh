...
bq query --nouse_legacy_sql \
  'SELECT DISTINCT year, month FROM kura.flights_raw ORDER BY year ASC, CAST(month AS INTEGER) ASC'

bq query --nouse_legacy_sql \
  'SELECT year, month, COUNT(*) AS num_flights FROM kura.flights_raw GROUP BY year, month ORDER BY year ASC, CAST(month AS INTEGER) ASC'
