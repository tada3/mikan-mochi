...
bq --project_id=$PROJECT query --destination_table ${DATASET}.flights_sample --replace --nouse_legacy_sql \
   "SELECT * FROM ${DATASET}.flights WHERE RAND() < 0.001"

bq --project_id=$PROJECT extract --destination_format=NEWLINE_DELIMITED_JSON \
   ${DATASET}.flights_sample  gs://${BUCKET}/flights/ch4/flights_sample.json
